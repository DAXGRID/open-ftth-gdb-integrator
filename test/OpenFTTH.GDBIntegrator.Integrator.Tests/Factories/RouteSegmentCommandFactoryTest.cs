...
namespace OpenFTTH.GDBIntegrator.Integrator.Tests.Factories
{
    public class RouteSegmentCommandFactoryTest
    {
        ...
        public async Task CreateUpdatedEvent_ShouldReturnRouteSegmentLocationChanged_OnGeometryChangedAndNodesBeingSameAsBeforeUpdate()
        {
            ...
            A.CallTo(() => geoDatabase.GetIntersectingStartRouteNodes(routeSegmentShadowTable.Coord)).Returns(new List<RouteNode> { previousStartNode });
            A.CallTo(() => geoDatabase.GetIntersectingEndRouteNodes(routeSegmentShadowTable.Coord)).Returns(new List<RouteNode> { previousEndNode });
